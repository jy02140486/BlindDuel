...
9. NPC 使用独立的轻量碰撞数据格式（`rootMotionOccupancyData`），由 `scripts/tools/extract_rootmotion_occupancy.ps1` 生成，仅含每帧 `anchors.root` + 固定尺寸 `occupancy.aabb`，不依赖 `.collider.json`。脚本必须扫 `Data/RootMotion/.../*.png`（root motion mask 专用 PNG，不是 `Art/Sprite` 复合 atlas——后者角色像素会覆盖 root 标记导致帧间扫描不一致）；`$OCCUPANCY_H` 固定为 `16`（不是 24），避免 AABB 中心在 root、高度过大下延导致碰撞范围虚大。
...
19. **Companion 碰撞与避障**（设计文档 `plans/archived/Companion 碰撞与避障设计.MD`，Step 1-4 已落地）：本游戏是 beat'em up 风格 2.5D（XY 二维移动，Y 是远近不是跳跃，无跳跃机制，walkArea 通常狭长矩形）。① Charlotte 同时进 `dynamicActors` + `staticBlockers`（`ExploreMode._buildIndices` 用 `if` 非 `else if` 允许双重身份，勿加 `!isCompanion` 排除）；② `NpcCharacter.isBlockingNow()` 基于 `_isFollowing` 标志（非 currentBehavior getter）：`blocksMovement && !_isFollowing` 才阻挡，`setFollowing` 由 `NpcController.startFollow`/`stop` 维护；③ `ExploreCollisionSystem.resolveMovement` 循环 blockers 必须 self-exclusion（`if (blocker === entity) continue;`），否则 companion 会阻挡自己，sequence 期间把她移到 walkArea 边界时会被挤出；④ `FollowingBehavior` 双轴 (X+Y) 跟随 + Y 轴 Separation：X 用 `followStart/Stop`，Y 用 `followDeadbandY/StopY`（player.y+区间停，不抠死同 y），Separation 的 `_sepDirY` 带 hysteresis（enter 时锁定 sign 避免穿越玩家时正负翻转抖动）；⑤ `ExploreCollisionSystem` debug mesh 必须挂 `entity.root` 作 parent（local position 按 AABB 中心偏移），否则 debug 框不跟随移动会误判碰撞范围虚大；⑥ `dynamicActors` 解算顺序固定 player → companion（player 先 push、先解算），companion 跟的是 player 本帧最新位置，避免同帧互相参考振荡。遗留 §10.3「Charlotte following 移动中被玩家挡」未实现（player 未进 blockers）。
20. **Animation pause/resume 设计约定**（TimelineSequencer 新增 clip）：`pauseAnimation` / `resumeAnimation` 只在 `FrameAnimationComponent` 层冻结帧推进，不改 CharacterBase 上层逻辑。`_consumeTransition` 正常执行但 `normalizedTime` 不变，依赖动画进度的 transition 条件自然 freeze。`enterState` 内部 `animation.play()` 会重置 `_paused = false`，所以 pause 期间发生状态切换会自动解除暂停，不会残留脏状态。详见 TimelineSequencer User Guide §5.16 / §5.17。
