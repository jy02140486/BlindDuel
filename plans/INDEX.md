...
- Audio System Step 5 完成：AudioBus 真正生效（设计稿 §11），`AudioManager._activeSounds: Map<Sound, meta>` 跟踪在播 Sound（`onEnded` 自动清理），`setBusVolume(name, value)` 遍历该 Map 实时重算 `finalVolume = baseVolume × busVolume × masterVolume`；bus 作为资源属性写入 `audio_clips.json` 的 def（符合「配置描述资源，代码描述行为」原则，调用方默认不感知 bus，`options.bus` 仅作例外覆盖）；localStorage 持久化 key=`blinduel_audio_bus_volumes`，构造时 load 覆盖配置默认值
- Step 5 新增 AmbientPlayer：多轨环境音管理（设计稿 §9.1 ambient 字段），lazy load + pending play（与 MusicPlayer 模式对称），`switchAmbient(ids)` 算差集停掉不在新列表里的、播放新增的、已在播的不动（避免重启）；`Scene.init` 末尾 `_applySceneAmbient` 读 `sceneDef.ambient`，`_resolveAmbientConfig` 支持 null/string/string[]/条件对象数组四种写法；`prologue.json` 配 `forest_ambient`（走 ambient bus）；`AssetManifest` 不改（ambient 复用 sfx 加载路径）
- AudioContext 解锁时机修复：`Scene.init` 时 ambient/music 在 suspended context 下 `sound.play()` 静默失败导致 source 丢失，后续 context resume 也不会自动补播；修复方式 `AudioManager._registerContextResumeListener` 监听 `pointerdown`/`keydown`/`audioEngine.onUnlock`，resume 后遍历 `_activeSounds` 对 `meta.loop === true` 的 sound 重新 `play()` 补播；用 `meta.loop` 判断而非 `sound.isLooping`（`BABYLON.Sound` 实例无此公开属性，Console 已证实返回 undefined）；只补播 loop sound（ambient/music），sfx 跳过避免已过期短音效被点出来；`_audioResumeDone` 标志防重复补播
